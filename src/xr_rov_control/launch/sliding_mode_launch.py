...
def generate_launch_description():
    ...
    with sl.group(ns=sl.arg('namespace')):
        ...
        sl.node('auv_control', 'sliding_mode', parameters=[sl.find('xr_rov_control', 'sliding_mode.yaml')],
                output='screen')
        ...
        with sl.group(if_arg='sliders'):
            ...
            sl.node('slider_publisher', 'slider_publisher', name='tilt_control',
                    arguments=[sl.find('xr_rov_control', 'tilt.yaml')])

    with sl.group(if_arg='rviz'):
        sl.include('xr_rov_control','rviz_launch.py',

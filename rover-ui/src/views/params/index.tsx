...
const ParamsView: React.FC<ParamsViewProps> = ({ isDarkMode = true }) => {
  ...
  // ==========================================
  // LOGIKA WARNA KONTRAS TINGGI (MIRIP PING SONAR)
  // ==========================================
  ...
  // Background solid putih untuk Light Mode
  const cardBg = isDarkMode ? 'bg-[#111827]/60 backdrop-blur-xl border-white/10 shadow-2xl' : 'bg-white border-slate-200 shadow-xl';
  ...
  const rowHover = isDarkMode ? 'hover:bg-white/[0.05]' : 'hover:bg-blue-50';
  ...
  const inputBg = isDarkMode
    ...
    : 'bg-white border-slate-300 text-blue-700 shadow-inner';

  const badgeBg = isDarkMode ? 'bg-white/10 text-slate-300 border-white/5' : 'bg-slate-100 border-slate-200 text-slate-700';
  const iconBoxBg = isDarkMode ? 'bg-black/30 border-white/10' : 'bg-white border-slate-200 shadow-sm';
    ...
    <div className="animate-in fade-in duration-500 pb-10 mt-2">
      <div className="max-w-7xl mx-auto w-full space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <div className={`p-1.5 rounded-lg transition-all border ${isDarkMode ? 'bg-white/5 border-white/5 group-hover:bg-blue-500/20' : 'bg-white border-slate-200 group-hover:bg-blue-50 shadow-sm'}`}>
              <ChevronLeft size={16} />
            </div>
        ...
        <div className={`border rounded-3xl overflow-hidden flex flex-col transition-all duration-300 ${cardBg}`}>
          ...
          {/* TOOLBAR */}
          <div className={`p-6 flex flex-col md:flex-row items-center justify-between gap-5 ${headerBg}`}>
            <div className="relative w-full md:w-96">
              <Search size={18} className={`absolute left-4 top-1/2 -translate-y-1/2 ${mutedText}`} />
              <input 
                type="text" 
                placeholder="Search parameter..." 
                className={`w-full rounded-xl py-3 pl-12 pr-4 text-sm outline-none transition-all border ${searchBg}`}
                value={searchTerm}
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <button 
                ...
                className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-xs font-bold border transition-all ${isDarkMode ? 'bg-white/5 hover:bg-white/10 border-white/10 text-slate-300' : 'bg-white hover:bg-slate-50 border-slate-300 text-slate-700 shadow-sm'}`}
              >
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className={`${borderColor} border-b ${isDarkMode ? '' : 'bg-slate-50/50'}`}>
                    ...
                    <tr key={param.id} className={`group transition-colors ${rowHover}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`p-2 rounded-lg border transition-colors ${iconBoxBg} ${mutedText} group-hover:text-blue-500 group-hover:border-blue-500/30`}>
                            <SlidersHorizontal size={14} />
                          </div>
                          <span className={`text-sm font-mono font-bold transition-colors ${isDarkMode ? 'text-slate-200' : 'text-slate-800'} ${tableTextHover}`}>
                            {param.name}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5">
                          ...
                          <span className={`inline-flex items-center w-fit px-2 py-0.5 rounded text-[9px] font-bold tracking-wider uppercase border ${badgeBg}`}>
                            {param.category}
                          </span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex justify-end items-center">
                          {param.type === 'toggle' ? (
                            <div className="flex items-center gap-3">
                              <span className={`text-[10px] font-bold tracking-widest uppercase transition-colors ${param.value ? 'text-emerald-600' : mutedText}`}>
                                {param.value ? 'Enabled' : 'Disabled'}
                              </span>
                              <button 
                                onClick={() => handleToggle(param.id)}
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ${param.value ? 'bg-emerald-500' : (isDarkMode ? 'bg-slate-600' : 'bg-slate-300')}`}
                              >
                            ...
                            <div className="relative group/input">
                              <input 
                                ...
                                className={`w-28 border focus:border-blue-400 rounded-lg py-2 px-3 text-sm font-mono text-right outline-none transition-all ${inputBg}`}
                              />
                            </div>
                          )}
                        </div>
          ...
          <div className={`p-4 border-t flex justify-between items-center text-[10px] font-mono uppercase tracking-widest ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'} ${mutedText}`}>

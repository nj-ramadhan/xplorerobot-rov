...
interface DepthControlProps {
  ...
  isDefault?: boolean; // Fix F: kalau true, label jadi "Default Depth"
}

export const DepthControl: React.FC<DepthControlProps> = ({ targetDepth, setTargetDepth, isDefault }) => {
  return (
    <div className="p-4 bg-black/40 rounded-xl border border-white/5">
      <div className="flex justify-between items-center mb-3">
        <label className="text-[10px] font-bold text-blue-400 uppercase tracking-wider">
{isDefault ? 'Default Depth (Z)' : 'Target Depth (Z)'}
        </label>
